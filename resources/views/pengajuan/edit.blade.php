...
<div class="container-fluid px-4">
    <h1 class="mt-4">Edit Pengajuan</h1>
            ...
            <form action="{{ route('pengajuan.update', $pengajuan->id_pengajuan) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="id_user" id="id_user" class="form-select @error('id_user') is-invalid @enderror">
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('id_user', $pengajuan->id_user) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="id_kelas" id="id_kelas" class="form-select @error('id_kelas') is-invalid @enderror">
                        ...
                        @foreach($kelas as $k)
                            <option value="{{ $k->id_kelas }}" {{ old('id_kelas', $pengajuan->id_kelas) == $k->id_kelas ? 'selected' : '' }}>
                                {{ $k->nama_kelas }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="id_barang" id="id_barang" class="form-select @error('id_barang') is-invalid @enderror">
                        ...
                        @foreach($barang as $b)
                            <option value="{{ $b->id_barang }}" {{ old('id_barang', $pengajuan->id_barang) == $b->id_barang ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="jumlah" id="jumlah" class="form-control @error('jumlah') is-invalid @enderror" value="{{ old('jumlah', $pengajuan->jumlah) }}" min="1">
                    @error('jumlah')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
